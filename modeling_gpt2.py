...
from transformers.modeling_tf_utils import (TFPreTrainedModel, TFConv1D, TFSharedEmbeddings,
...
class TFGPT2LMHeadModel(TFGPT2PreTrainedModel):
    ...
    def call(self, inputs, **kwargs):
        ...
        return outputs[0]  # lm_logits, presents, (all hidden_states), (attentions)
...
class TFGPT2DoubleHeadsModel(TFGPT2PreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.transformer = TFGPT2MainLayer(config, name='transformer')
        self.multiple_choice_head = TFSequenceSummary(config, initializer_range=config.initializer_range,
    ...
    def call(self, inputs, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
             inputs_embeds=None, mc_token_ids=None, training=False):
        if isinstance(inputs, (tuple, list)):
            input_ids = inputs[0]
            past = inputs[1] if len(inputs) > 1 else past
            attention_mask = inputs[2] if len(inputs) > 2 else attention_mask
